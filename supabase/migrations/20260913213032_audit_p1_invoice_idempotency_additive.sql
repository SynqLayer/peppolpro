-- P1-03 ADDITIVE: compatible with the existing billing RPC signature.
-- Abort on unexpected pre-existing duplicates; never delete/re-number invoices.
create unique index invoices_payment_original_unique on public.invoices(payment_id)
 where payment_id is not null and invoice_kind in ('credits','subscription');
create unique index invoices_payment_legacy_credit_unique on public.invoices(payment_id)
 where payment_id is not null and invoice_kind = 'credit';
...
begin
 ...
 -- The payment row is already locked: retries return the same identity before
 -- mutable profile validation or allocation of another invoice number.
 select i.id, i.invoice_number into v_invoice_id, v_invoice_number
 from public.invoices i
 where i.payment_id = v_payment.id
  and ((v_invoice_kind = 'credit' and i.invoice_kind = 'credit')
   or (v_invoice_kind <> 'credit' and i.invoice_kind in ('credits','subscription')));
 if found then
