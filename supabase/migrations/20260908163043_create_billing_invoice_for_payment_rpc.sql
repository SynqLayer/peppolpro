-- Spiegelbestand van de productiehistorie (versie 20260908163043, create_billing_invoice_for_payment_rpc).
--
...
begin
  ...
  select * into v_payment from public.payments where payments.id = p_payment_id for update;
  if not found then
  ...
  select * into v_profile from public.user_profiles where user_profiles.id = v_payment.user_id for update;
  if not found then
  ...
  if v_invoice_kind = 'credit' then
    ...
    where invoices.payment_id = v_payment.id and invoices.invoice_kind <> 'credit'
