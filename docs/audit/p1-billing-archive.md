...
P1-03 follow-up: the shared PostgreSQL harness now waits for TCP pg_isready plus SELECT 1 (250 ms polling, 60 second deadline), avoiding the image's temporary Unix-socket initialization server. Each database is network-isolated with a dedicated internal port and tmpfs storage. Only stale audit-owned containers are removed. Infrastructure failures retry at most three times and include logs; assertions and SQL correctness failures are never retried or skipped.
